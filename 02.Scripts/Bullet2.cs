...
public class Bullet2 : MonoBehaviourPun
{
    ...
    void DestroyEffect()
    {
        Debug.Log("DestroyEffect() È£Ãâ");
        GameObject[] effect2 = GameObject.FindGameObjectsWithTag("EFFECT2");
        foreach (GameObject go in effect2)
            Destroy(go, 2f);
    }
    ...
    void HitTarget()
    {
        GameObject effectIns2 = PhotonNetwork.Instantiate(impactEffect2.name, transform.position, transform.rotation);
        //photonView.RPC("DestroyEffect", RpcTarget.All);
